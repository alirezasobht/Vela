...
class AssetRepositoryImplTest {
    ...
    private fun aCoinDto(id: String = "bitcoin") =
        CoinDto(
            id = id,
            symbol = "btc",
            name = "Bitcoin",
    ...
    private fun anAssetEntity(id: String = "bitcoin") =
        AssetEntity(
            id = id,
            symbol = "btc",
            name = "Bitcoin",
    ...
    @Test
    fun `getTopAssets emits assets from database`() =
        runTest {
    ...
    @Test
    fun `getTopAssets handles empty database state`() =
        runTest {
    ...
    @Test
    fun `getTopAssets applies limit parameter to database query`() =
        runTest {
    ...
    @Test
    fun `getTopAssets emits database updates in real time`() =
        runTest {
    ...
    @Test
    fun `getTopAssets mapping integrity from Entity to Domain`() =
        runTest {
    ...
    @Test
    fun `refresh successfully updates local database`() =
        runTest {
    ...
    @Test
    fun `refresh returns DataResult Success Unit on completion`() =
        runTest {
            coEvery { api.getMarkets(limit = any()) } returns listOf(aCoinDto())
            coEvery { dao.refresh(any()) } just runs
    ...
    @Test
    fun `refresh Network connectivity failure`() =
        runTest {
    ...
    @Test
    fun `refresh Generic exception handling`() =
        runTest {
    ...
    @Test
    fun `refresh Exception with null message handling`() =
        runTest {
    ...
    @Test
    fun `fetchTopAssets handles database transaction failure`() =
        runTest {
            coEvery { api.getMarkets(limit = any()) } returns listOf(aCoinDto())
    ...
    @Test
    fun `refresh Limit parameter integrity`() =
        runTest {
    ...
    @Test
    fun `refresh handles zero or negative limit values`() =
        runTest {
    ...
    @Test
    fun `refresh Nullable fields in CoinDto are handled`() =
        runTest {
    ...
    @Test
    fun `refresh mapping integrity from DTO to Entity`() =
        runTest {
            val dto = aCoinDto()
            coEvery { api.getMarkets(limit = any()) } returns listOf(dto)
