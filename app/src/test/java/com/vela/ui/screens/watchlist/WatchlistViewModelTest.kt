...
class WatchlistViewModelTest {
    ...
    @Test
    fun `empty watchlist results in Empty state`() = runTest {
    ...
    @Test
    fun `non-empty watchlist results in Success with mapped models`() = runTest {
    ...
    @Test
    fun `assets are cached in AssetPreviewCache`() = runTest {
        val assets = listOf(asset("bitcoin"), asset("ethereum"))
        every { getWatchlistAssets() } returns flowOf(DataResult.Success(assets))
    ...
    @Test
    fun `error results in Empty state`() = runTest {
