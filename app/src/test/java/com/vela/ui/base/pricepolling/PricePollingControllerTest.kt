...
class PricePollingControllerTest {
    ...
    @Test
    fun `polling does not fire before delay`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns DataResult.Success(emptyMap())
    ...
    @Test
    fun `polling pauses when screen not visible`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns DataResult.Success(emptyMap())
    ...
    @Test
    fun `polling resumes when screen becomes visible`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns DataResult.Success(emptyMap())
    ...
    @Test
    fun `on success delegate onPriceError called with null`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns
                DataResult.Success(
    ...
    @Test
    fun `on error delegate onPriceError called with error`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
    ...
    @Test
    fun `polling stops after cancel`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns DataResult.Success(emptyMap())
    ...
    @Test
    fun `observePrice emits null for unknown id`() =
        runTest {
            val controller = PricePollingController(config)
    ...
    @Test
    fun `getIds returning empty list skips getPrices call`() =
        runTest {
    ...
    @Test
    fun `bind called twice does not cause double polling`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns DataResult.Success(emptyMap())
    ...
    @Test
    fun `error then success clears onPriceError`() =
        runTest {
            coEvery { delegate.getIds() } returns listOf("bitcoin")
            coEvery { getPrices(any()) } returns DataResult.Error(AppError.NoInternet)
