...
class PricePollingController @Inject constructor(private val config: PricePollingConfig) : PricePolling {
    ...
    fun bind(
        ...
        cancel()
        pollingScope = CoroutineScope(scope.coroutineContext + SupervisorJob()).apply {
    ...
    override fun observePrice(id: String): Flow<SimplePriceUiModel?> = prices
        .map { it[id]?.toUiModel() }
