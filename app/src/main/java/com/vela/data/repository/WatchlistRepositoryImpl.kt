...
class WatchlistRepositoryImpl @Inject constructor(private val dao: WatchlistDao) : WatchlistRepository {
    override fun observeWatchlist(): Flow<List<String>> = dao.observeAll().map { list -> list.map { it.coinId } }

    override fun isWatchlisted(coinId: String): Flow<Boolean> = dao.isWatchlisted(coinId)
